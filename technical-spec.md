...
Отдельный Gradle-скрипт или shell-скрипт `prepare-lesson` принимает имя и фамилию лида и:

1. Проверяет, что исходная ветка `main` существует и рабочее дерево не содержит незакоммиченных изменений.
2. Нормализует имя в безопасный slug.
3. Создаёт новую ветку вида `lesson/имя-фамилия-дата` от `main`.
4. Создаёт отдельный Git worktree для этой ветки, чтобы `.env`, SQLite и файлы одного занятия не попадали в следующее.
5. Копирует `.env.example` в новый локальный `.env` без токенов.
6. Создаёт чистый каталог `data`; Flyway создаёт базу при первом запуске.
7. Выводит путь к готовому проекту, который нужно открыть в IntelliJ IDEA.
...
- проект запускается одной кнопкой из IntelliJ на подготовленном ноутбуке;
...
- `prepare-lesson` создаёт отдельную ветку, worktree, чистый `.env` и новую SQLite-базу;
...
9. Скрипт `prepare-lesson`, отдельные ветки/worktree и пул школьных токенов.
...
- [x]  Ассеты: **PNG**.
- [x]  Подготовка занятия: скрипт создаёт новую ветку от `main` по имени и фамилии лида и отдельный чистый worktree.
...
- [ ]  точный интерфейс и платформа запуска `prepare-lesson`.