...
Ты не создаёшь бот с нуля: ты дорабатываешь существующий продукт. В выбранном маршруте есть четыре небольших GitHub Issues — два видимых бага и две функции. На одном уроке достаточно качественно решить один баг и одну функцию.
...
2. Открыть свой небольшой файл: `StudyLesson.java`, `SportLesson.java` или `BlogLesson.java` в каталоге [`lesson/route`](src/main/java/studio/pixelpoint/magicpet/lesson/route).
...
Карта трёх маршрутов и двенадцать карточек находятся в [`lesson/README.md`](lesson/README.md). На одном уроке применяется только один patch.
...
Скопируй команду из раздела «Готово, если» своей карточки. Она запускает одну проверку и не показывает ошибки оставшихся задач.

## Как посмотреть свои изменения

В терминале выполни `git diff`. Зелёные строки добавлены, красные удалены. Перед commit убедись, что менялся только твой учебный файл и разрешённые карточкой ресурсы.
...
Маршрут обязателен: `study`, `sport` или `blog`. Скрипт создаёт отдельную ветку и worktree, чистые `.env`/`data`, применяет ровно один учебный patch и выводит путь, который нужно открыть в IntelliJ.
...
`final-acceptance.sh` намеренно запускается только из чистого commit ветки `main`. Ручные замеры Telegram и урока фиксируются в [`acceptance/PILOT-REPORT.md`](acceptance/PILOT-REPORT.md).
